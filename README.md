...
## Меры защиты
- Аутентификация: JWT c `sub` и `exp`; проверка в декораторе `login_required`, привязка пользователя к `g.current_user`.
- Защита паролей: `bcrypt` (salt+hash), верификация через `checkpw`, пароли в БД не хранятся открыто.
